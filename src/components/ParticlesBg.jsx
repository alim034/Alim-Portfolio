...
const pathVariants = {
  initial: { strokeDashoffset: 620, strokeDasharray: '26 760' },
  animate: {
    strokeDashoffset: 0,
    strokeDasharray: '12 740',
    opacity: [0.2, 0.95, 0.95, 0.2],
  },
};

function BackgroundLines({ className = "", svgOptions }) {
  return (
...
function SVG({ svgOptions }) {
  const paths = [
    ...
    // ... remaining paths shortened for brevity in code patch
  ];
  ...
  const colors = [
    ...
    // extra vibrant hues for richer palette
    '#22D3EE', '#3B82F6', '#8B5CF6', '#A855F7', '#06B6D4', '#10B981', '#F59E0B', '#EF4444'
  ];
  ...
  return (
    ...
    >
      ...
      {paths.map((path, idx) => (
        <motion.path
          d={path}
          stroke={colors[(idx + 14) % colors.length]}
          strokeWidth="0.9"
          strokeLinecap="round"
          variants={pathVariants}
          initial="initial"
          animate="animate"
          transition={{
            duration: svgOptions?.duration || 4.5,
            ease: 'linear',
            repeat: Infinity,
            repeatType: 'loop',
