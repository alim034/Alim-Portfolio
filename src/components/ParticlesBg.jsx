...
export default function ParticlesBg() {
  ...
  return (
    <div aria-hidden className="fixed inset-0 -z-10 pointer-events-none overflow-hidden">
      ...
      {lightMode ? (
        <motion.div
          animate={{ x: ['-10%', '5%', '-10%'], y: ['-8%', '6%', '-8%'], scale: [1, 1.08, 1] }}
          transition={{ duration: 22, repeat: Infinity, ease: 'easeInOut' }}
          className="absolute top-[-30%] left-[-30%] w-[360px] h-[360px] md:w-[500px] md:h-[500px] rounded-full opacity-35"
          style={{
            background:
              'radial-gradient(circle, rgba(59,130,246,0.5) 0%, rgba(59,130,246,0.2) 40%, rgba(59,130,246,0.08) 70%, transparent 100%)',
            filter: 'blur(40px)'
          }}
        />
      ) : (
        <motion.div
          ...
          className="absolute top-[-20%] left-[-20%] w-[500px] h-[500px] rounded-full opacity-40 transform-gpu"
          style={{
            background:
      ...
      {lightMode ? (
        <motion.div
          animate={{ x: ['8%', '-6%', '8%'], y: ['10%', '-6%', '10%'], scale: [1.05, 0.95, 1.05] }}
          transition={{ duration: 26, repeat: Infinity, ease: 'easeInOut' }}
          className="absolute bottom-[-30%] right-[-30%] w-[300px] h-[300px] md:w-[400px] md:h-[400px] rounded-full opacity-30"
          style={{
            background:
              'radial-gradient(circle, rgba(139,92,246,0.6) 0%, rgba(139,92,246,0.3) 40%, rgba(139,92,246,0.08) 70%, transparent 100%)',
            filter: 'blur(35px)'
          }}
        />
      ) : (
        <motion.div
          ...
          className="absolute bottom-[-20%] right-[-20%] w-[400px] h-[400px] rounded-full opacity-35 transform-gpu"
          style={{
            background:
      ...
      {!lightMode && (
        <motion.div
          ...
          className="absolute top-[20%] left-[30%] w-80 h-80 rounded-full hidden md:block transform-gpu"
          style={{
            background:
      ...
      {!lightMode && (
        <motion.div
          ...
          className="absolute bottom-[25%] right-[25%] w-72 h-72 rounded-full hidden md:block transform-gpu"
          style={{
            background:
      ...
      {Array.from({ length: lightMode ? 6 : 15 }).map((_, i) => {
        const randomDelay = Math.random() * 10
        const randomDuration = (lightMode ? 16 : 10) + Math.random() * (lightMode ? 10 : 15)
        const randomSize = (lightMode ? 18 : 30) + Math.random() * (lightMode ? 28 : 60)
        return (
          <motion.div
            key={`particle-${i}`}
            animate={lightMode ? {
            ...
            className={`absolute rounded-full ${!lightMode && i >= 5 ? 'hidden md:block' : ''} transform-gpu`}
            style={{
              left: `${10 + i * 6}%`,
              top: `${5 + i * 7}%`,
              width: `${randomSize}px`,
